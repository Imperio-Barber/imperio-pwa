...
export default function AdminPage() {
  ...
  const inputClass =
    'w-full min-h-[48px] rounded-2xl border border-white/25 bg-zinc-950/80 px-4 py-3 text-white placeholder:text-zinc-500 outline-none transition focus:border-[#7bc892] focus:ring-2 focus:ring-[#7bc892]/30'

  const tileClass =
    'rounded-3xl border border-white/10 bg-zinc-950/70 p-4 shadow-xl backdrop-blur-sm'
  ...
  async function loadAdminData() {
    ...
    const [employeesResult, transactionsResult, closuresResult, servicesResult, productsResult] =
      await Promise.all([
    ...
    const nextRows: EmployeeRow[] = (employeesResult.data as Employee[]).map((employee) => ({
      employee,
      transactions: typedTransactions.filter(
  ...
  useEffect(() => {
    const savedAdminLogin = sessionStorage.getItem('imperioAdminLogged')
  ...
  function loginAdmin() {
    ...
    sessionStorage.setItem('imperioAdminLogged', 'true')
    setIsLoggedIn(true)
    setAdminPin('')
    setMessage('')
  }
  ...
  function logoutAdmin() {
    sessionStorage.removeItem('imperioAdminLogged')
    setIsLoggedIn(false)
    setAdminPin('')
    setMessage('')
  }
  ...
  async function updateProductName(product: Product, value: string) {
    ...
    const { error } = await supabase.from('products').update({ name }).eq('id', product.id)
  ...
  async function updateServicePrice(service: Service, value: string) {
    ...
    const { error } = await supabase.from('services').update({ price }).eq('id', service.id)
      ...
      >
        <div className="min-h-screen bg-black/25">
          <div className="mx-auto min-h-screen max-w-5xl p-4 md:p-8">
            ...
            <section className={`${tileClass} mx-auto max-w-md`}>
              <div>
  ...
  return (
    ...
    >
      <div className="min-h-screen bg-black/25">
        <div className="mx-auto min-h-screen w-full max-w-[1600px] overflow-x-hidden p-2 sm:p-4 md:p-6">
          ...
          {editingTransaction ? (
            <section className={`${tileClass} mb-4 border-imperio-gold/40`}>
              <div className="mb-4">
                <p className="text-sm text-imperio-gold">Korekta administratora</p>
                <h2 className="text-2xl font-bold">
                  {editServiceName || editingTransaction.service_name}
                </h2>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    inputMode="decimal"
                    value={editTipAmount}
                    onChange={(event) => setEditTipAmount(event.target.value)}
                  />
                </div>
          ...
          <section className="mt-4 grid gap-4">
            {rows.map((row) => {
              ...
              return (
                <div className={tileClass} key={row.employee.id}>
                  <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-white/50">
                        Transakcje: {row.transactions.length}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 md:items-end">
                      <div
                        className={`rounded-xl px-4 py-2 text-sm font-semibold ${
                          row.closure ? 'bg-green-500/15 text-green-200' : 'bg-red-500/15 text-red-200'
                        }`}
                      >
                      ...
                      {row.closure ? (
                        <button
                          ...
                          type="button"
                          onClick={() => unlockDay(row.employee.id)}
                        >
                          Odblokuj dzień
                        </button>
                      ) : (
                  ...
                  <div className="mb-4 grid gap-3 lg:grid-cols-[1fr_160px_120px] lg:items-end">
                    <div className="text-sm text-white/50">
                      {row.employee.name === 'Ola'
                        ? 'Model wypłaty: procent od netto + połowa VAT.'
                        : 'Model wypłaty: procent od brutto.'}
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onBlur={(event) => updateCommission(row.employee.id, event.target.value)}
                      />
                    </div>
                    ...
                    <button
                      className="btn btn-dark"
                      type="button"
                      disabled={savingEmployeeId === row.employee.id}
          ...
          <section className={`${tileClass} mt-4`}>
            <div className="mb-4">
...
function ProductRow({
  ...
  return (
    <tr className="border-t border-white/10">
      ...
      <td className="px-3 py-3 font-bold">{product.stock_quantity} szt.</td>
